...
autos_data[problematic_rows,] <- autos_data %>% 
  ...
  unite(col="all", -seq(1:26), sep = "/", remove=TRUE) %>% 
  extract(all, into=c("curb-weight", "engine-type", "resto"), regex="([0-9]+)([a-z]+)/(.*)/NA", remove=TRUE) %>% 
  separate(resto, into=names(autos_data)[9:18], sep="/", remove=TRUE) %>% 
  readr::type_convert() %>% 
  mutate(curb-weight=as.character(curb-weight))
